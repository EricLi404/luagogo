...
func TestDump(t *testing.T) {
	...
	// upvalues (1):
	// 0	_ENV	1	0
	data, err := ioutil.ReadFile("./testdata/hello_world.luac.out")
	if err != nil {
		panic(err)
	}
	proto := Dump(data)
...
func printDetail(f *Prototype) {
	...
	for i, upval := range f.Upvalues {
		fmt.Printf("\t%d\t%s\t%d\t%d\n",
			i, upvalueNames(f, i), upval.Instack, upval.Idx)
	}
}
...
func upvalueNames(f *Prototype, idx int) string {
